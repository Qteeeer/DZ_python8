...
def show_data(): #содержмое
    with open('data.txt', 'r', encoding='utf-8') as file:
        book = file.read()  # .split('\n')
    return book


def new_data(): # Добавление стоки
    with open('data.txt', 'a', encoding='utf-8') as file:
        file.write(input('Введите новую строку: ' + '\n'))


def find_data(): # Поиск информации
    with open('data.txt', 'r', encoding='utf-8') as file:
        book = file.read().split('\n')
        temp = input('что ищем?: ')
        for i in book:
...
def delete_person(name): # Удаление
    persons = read_data()
...
def change_person(new_name, old_name):  # Изменение
    persons = read_data()
    with open("data.txt", "w", encoding="utf8") as file:
        for person in persons:
            if old_name != person:
